...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  global data_index
  # win_sz = 2*skip_window+1
  con_index = data_index+skip_window  
  # print("start data index", data_index)
  count = 0   
  ...
  while(count<batch_size):    
    tmp_count = 0 
    # print(data_index,data_index+win_sz-1) 
    # print(data[data_index:data_index+win_sz])
    l= int(con_index-(num_skips/2))
    r= int(con_index+(num_skips/2))
    # print("left", l)    
    # print("right", r)    
    for i in range(l,r+1):      
      ...
      #   indx = con_index+r
      #   r=r+1
      #   if(indx>(data_index+2*skip_window)):
      #     indx = con_index-l
      #     l=l-1

      # if(indx<data_index or indx>(data_index+2*skip_window) or indx<0 or indx > len(data)):
      #   break
      # tmp_count +=1
      # batch[count] = data[con_index]
      # labels[count] = data[indx]
      # count=count+1
      # if(data_index+curr == con_index):
      #   curr = curr+1
      #   continue
...
if __name__ == '__main__':
  ...
  batch, labels = generate_batch(data, batch_size=4, num_skips=2, skip_window=1)
  for i in range(4):
  ...
  # We pick a random valid  ation set to sample nearest neighbors. Here we limit the
