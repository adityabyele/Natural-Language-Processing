...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
            ...
            hidden_size3 = 100
            weights_input3 = tf.Variable(tf.random_normal((hidden_size3, hidden_size2), 0, 0.1))
            print "w ip3", weights_input3.shape
            biases_input3 = tf.Variable(tf.zeros((hidden_size3, 1)))
            print "b ip3", biases_input3.shape            
            weights_output = tf.Variable(tf.random_normal((parsing_system.numTransitions(), hidden_size3), 0, 0.1))
            ...
            #
            # self.train_labels = 
            self.prediction = self.forward_pass(embed, weights_input, biases_input, weights_input2, biases_input2, weights_input3, biases_input3, weights_output)
            print "pred", self.prediction.shape
            wt23_l2 = tf.nn.l2_loss(weights_input2)+tf.nn.l2_loss(biases_input2)+tf.nn.l2_loss(weights_input3)+tf.nn.l2_loss(biases_input3)
            l2 = wt23_l2+tf.nn.l2_loss(weights_input)+ tf.nn.l2_loss(weights_output)+tf.nn.l2_loss(biases_input)+tf.nn.l2_loss(self.embeddings)
            ...
            self.test_pred = self.forward_pass(test_embed, weights_input, biases_input, weights_input2, biases_input2, weights_input3, biases_input3, weights_output)
    ...
    def forward_pass(self, embed, weights_input, biases_inpu,  weights_input2, biases_input2, weights_input3, biases_input3, weights_output):
        """
        ...
        ##############Third Hidden Layer#################
        wi3 = tf.transpose(weights_input3)
        print "wi3", wi3.shape
        b3 = tf.transpose(biases_input3)
        print "b3", b3.shape
        hidd_op3 = tf.math.add(tf.matmul(hidd_op2, wi3), b3)
        print "hidd_op3", hidd_op3.shape        
        # Best Configuration
        # hidd_op3 = tf.tanh(hidd_op3)
        hidd_op3 = tf.pow(hidd_op3, 3)

        #############Output Layer#########################
        wo = tf.transpose(weights_output)
        print "wo", wo.shape
        op = tf.matmul(hidd_op3, wo)
        print "op", op.shape
