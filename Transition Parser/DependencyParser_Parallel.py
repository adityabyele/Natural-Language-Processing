...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
            ...
            w1 = tf.Variable(tf.random_normal((75, Config.embedding_size*18), 0, 0.1))
            print "w1 ip", w1.shape
            b1 = tf.Variable(tf.zeros((75, 1)))
            print "b1 ip", b1.shape            
            w2 = tf.Variable(tf.random_normal((75, Config.embedding_size*18), 0, 0.1))
            print "w2 ip", w2.shape
            b2 = tf.Variable(tf.zeros((75, 1)))
            print "b2 ip", b2.shape            
            w3 = tf.Variable(tf.random_normal((50, Config.embedding_size*12), 0, 0.1))
            print "w3 ip", w3.shape
            b3 = tf.Variable(tf.zeros((50, 1)))
            print "b3 ip", b3.shape            
            weights_output = tf.Variable(tf.random_normal((parsing_system.numTransitions(), Config.hidden_size), 0, 0.1))
            ...
            #
            # self.train_labels = 
            self.prediction = self.forward_pass(embed, w1,b1,w2,b2,w3,b3, weights_output)
            print "pred", self.prediction.shape
            l2_wt = tf.nn.l2_loss(w1)+tf.nn.l2_loss(w2)+tf.nn.l2_loss(w3)+tf.nn.l2_loss(b1)+tf.nn.l2_loss(b2)+tf.nn.l2_loss(b3)
            l2 = l2_wt+tf.nn.l2_loss(weights_output)+tf.nn.l2_loss(self.embeddings)
            ...
            self.test_pred = self.forward_pass(test_embed, w1,b1,w2,b2,w3,b3, weights_output)
    ...
    def forward_pass(self,embed, w1,b1,w2,b2,w3,b3, weights_output):
        """
        ...
        =======================================================
        """
        e1, e2, e3 = tf.split(embed, [Config.embedding_size*18, Config.embedding_size*18, Config.embedding_size*12], 1)
        ...
        bi1 = tf.transpose(b1)
        print "bi1", bi1.shape
        hidd_op1 = tf.math.add(tf.matmul(e1, wi1), bi1)
        print "hidd_op1", hidd_op1.shape
        #cubic activation
        hidd_op1 = tf.pow(hidd_op1, 3)
        #############################################################
        ...
        bi2 = tf.transpose(b2)
        print "bi2", bi2.shape
        hidd_op2 = tf.math.add(tf.matmul(e2, wi2), bi2)
        print "hidd_op2", hidd_op2.shape
        #cubic activation
        hidd_op2 = tf.pow(hidd_op2, 3)
        #############################################################
        ####################Third Parallel layer####################
        wi3 = tf.transpose(w3)
        print "wi3", wi3.shape
        bi3 = tf.transpose(b3)
        print "bi3", bi3.shape
        hidd_op3 = tf.math.add(tf.matmul(e3, wi3), bi3)
        print "hidd_op3", hidd_op3.shape
        #cubic activation
        hidd_op3 = tf.pow(hidd_op3, 3)
        #############################################################
        hidd_op = tf.concat([hidd_op1,hidd_op2,hidd_op3], axis = 1)
        print "hidd_op", hidd_op.shape
        wo = tf.transpose(weights_output)
        print "wo", wo.shape
        op = tf.matmul(hidd_op, wo)
        print "op", op.shape
