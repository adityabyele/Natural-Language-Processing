...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
            ...
            l2 = tf.nn.l2_loss(weights_input)+ tf.nn.l2_loss(weights_output)+tf.nn.l2_loss(biases_input)+tf.nn.l2_loss(self.embeddings)
    ...
    def forward_pass(self, embed, weights_input, biases_inpu, weights_output):
        ...
        #2a relu activation
        # hidd_op = tf.nn.relu(hidd_op)
        # 2a tanh activation
        # hidd_op = tf.tanh(hidd_op)
        # 2a sigmoid
        # hidd_op = tf.sigmoid(hidd_op)
